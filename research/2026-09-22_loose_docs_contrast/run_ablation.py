"""Retrieval ablation, loose-docs contrast: same 5 configs, 3-doc corpus.

NOTE (context): this is the v1 ablation script logic UNCHANGED from
research/2026-09-22_alias_boost_ablation/run_ablation.py, run against a
regenerated metadata/chunks.json that adds two loose-docs session reviews
(data/session_review_1.md, data/session_review_2.md). Those docs share
topical vocabulary with the gold queries but NOT the section_id slugs the
alias table hardcodes. chunks.json is restored to its frozen bytes after
the run (git checkout -- metadata/chunks.json).
...
def _load_frozen_store():
    """Read chunks.json only. Never ingests — the 3-doc index must already
    exist (run ingest.py with all three docs present first)."""
    store = load_store()
    if store is None:
        raise RuntimeError(
